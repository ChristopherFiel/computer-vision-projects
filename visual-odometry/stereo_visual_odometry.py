...
class VisualOdometry:
    def __init__(self, data_dir):
        ...
        self.disparities = [
            np.divide(
                self.disparity.compute(self.images_l[0], self.images_r[0]).astype(
                    np.float32
                ),
                16,
            )
    ...
    def get_tiled_keypoints(self, img, tile_h, tile_w):
        ...
        kp_list = [
            get_kps(x, y) for y in range(0, h, tile_h) for x in range(0, w, tile_w)
        ]
    ...
    def track_keypoints(self, img1, img2, kp1, max_error=4):
        ...
        h, w = img1.shape
        in_bounds = np.where(
            np.logical_and(trackpoints2[:, 1] < h, trackpoints2[:, 0] < w), True, False
        )
    ...
    def calculate_right_qs(self, q1, q2, disp1, disp2, min_disp=0.0, max_disp=100.0):
        ...
        def get_idxs(q, disp):
            ...
            return disp, np.where(
                np.logical_and(min_disp < disp, disp < max_disp), True, False
            )
    ...
    def estimate_pose(self, q1, q2, Q1, Q2, max_iter=100):
        ...
        for _ in range(max_iter):
            ...
            sample_q1, sample_q2, sample_Q1, sample_Q2 = (
                q1[sample_idx],
                q2[sample_idx],
                Q1[sample_idx],
                Q2[sample_idx],
    ...
    def get_pose(self, i):
        ...
        self.disparities.append(
            np.divide(
                self.disparity.compute(img2_l, self.images_r[i]).astype(np.float32), 16
            )
        )
