...
def visualize_paths(
    gt_path, pred_path, html_tile="", title="VO exercises", file_out="plot.html"
):
...
def make_residual_plot(x, residual_init, residual_minimized):
    ...
    plot_data = ColumnDataSource(
        data={"x": x, "residual": residual_minimized, "change": change}
    )
    tooltips = [
        ("change", "@change"),
    ]
...
def plot_residual_results(
    ...
    fig1, fig2, fig3 = make_residual_plot(
        x, small_residual_init, small_residual_minimized
    )
    ...
    show(
        layout(
            [
                ...
                Div(
                    text="<h2>Bundle adjustment with all parameters (with sparsity)</h1>"
                ),
