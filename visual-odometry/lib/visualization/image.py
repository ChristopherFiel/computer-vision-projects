...
def create_face_collage(
    image, faces, fontScale=0.5, text_thickness=1, face_size=(100, 100)
):
    faces_sub = []
    for i, (v, u, w, h) in enumerate(faces):
    ...
    sub_faces = np.zeros(
        shape=(face_size[0] * rows, face_size[1] * cols, 3), dtype=np.uint8
    )
    for i, face_sub in enumerate(faces_sub):
        ...
        sub_faces[
            face_size[0] * r : face_size[0] * (r + 1),
            face_size[1] * c : face_size[1] * (c + 1),
        ] = face_sub
    return sub_faces
